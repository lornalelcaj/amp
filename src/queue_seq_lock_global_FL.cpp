...
QueueSequentialLockGlobalFL::node_t* QueueSequentialLockGlobalFL::FreeList::pop(queue_stats_t *stats) {
    node_t *n = this->head;
    if (n) {
        this->head = n->next;
        this->cur_size--;
...
void QueueSequentialLockGlobalFL::FreeList::push(node_t *n, queue_stats_t *stats) {
    n->next = this->head;
    this->head = n;
    this->cur_size++;
